<template>
  <div class="imx-fe">
    <div v-if="$route.meta.name === 'imx-be'">
      <div class="col s12 l8">
        <div class="card" ref="my_builds">
          <div class="card-content">
            <span class="card-title">My IMX BE builds</span>
            <Builds :created-by="this.$auth.getUser().username" module="imx_be"></Builds>
          </div>
        </div>
        <div class="card" ref="builds_by_branch">
          <div class="card-content">
            <span class="card-title">Active iMX BE builds by branch</span>
            <table>
              <thead>
              <tr>
                <th>#</th>
              ...
              <tr v-for="(build, index) in activeBuildsGroupedByBranch" :key="index">
                <td>{{ index + 1 }}</td>
                <td>
                  <router-link :to="'/imx_be/branches?branch=' + encodeURIComponent(build.branch)">
                    {{ build.branch }}
                  </router-link>
                </td>
                <td>
      ...
      <div class="col s12 l4">
        <div class="card" ref="stats_by_user">
          <div class="card-content">
            <span class="card-title">iMX BE builds by user</span>
        ...
        <div class="card" ref="stats_by_branch">
          <div class="card-content">
            <span class="card-title">iMX BE builds by branch</span>
            <div class="col s12 l6 right">
              <div class="input-field">
                <Select class="col s12"
                        icon="today"
...
export default {
  ...
  computed: {
    userActiveBuilds() {
      return this.$store.getters['builds/getActiveByUser'](this.$auth.getUser().username, 'imx_be');
    },
    activeBuildsGroupedByBranch() {
      return this.$store.getters['builds/getActiveGroupedByBranch']('imx_be');
    },
    branchesChartData() {
      const builds = this.$store.getters['builds/getByBranch']('imx-fe-branch-builds', 'imx_be');
    ...
    usersChartData() {
      const builds = this.$store.getters['builds/getByUser']('imx-fe-users-builds', 'imx_be');
